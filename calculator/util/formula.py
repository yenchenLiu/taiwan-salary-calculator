class Salary:
    ...
    salary_levels = []
    with open("./calculator/util/salary_level.csv", "r") as f:
        lines = f.readlines()
...
if __name__ == "__main__":
    ...
    print(
        f"實領薪資:{salary_input - (s.get_total_labor() + s.get_health_insurance())}")
    ...
    print(
        f"公司負擔小計:{c.get_total_labor() + c.get_health_insurance() + c.get_pension()}")
    print(
        f"公司支出:{c.get_total_labor() + c.get_health_insurance() + c.get_pension() + salary_input}")
